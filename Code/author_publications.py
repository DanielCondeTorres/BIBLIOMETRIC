...
def visualize_top_authors_by_pubs(top_authors):
    """Generates visualization of the top 10 authors by total publications"""
    plt.figure(figsize=(14, 10))
    sns.set_theme(style="whitegrid")

    # Bar plot horizontal by Total Publications
    ax = sns.barplot(
    ...
    # Customization
    plt.title('Top 10 Authors by Total Publications', fontsize=18, fontweight='bold', pad=20)
    plt.xlabel('Total Publications', fontsize=22, fontweight='bold')
    plt.ylabel('', fontsize=22)
    plt.xlim(0, top_authors['Total Publications'].max() + 5)
...
if __name__ == "__main__":
    ...
    # Get top 10 by number of publications
    top_10_pubs = authors_df.sort_values('Total Publications', ascending=False).head(10)

    # Show results
    print("\n🏆 Top 10 Authors by Total Publications:")
    print(
        top_10_pubs[['Author', 'Total Publications', 'H-Index', 'Total Citations']]
        .reset_index(drop=True)
        .to_string(index=False)
    )

    # Generate visualization
    visualize_top_authors_by_pubs(top_10_pubs)
